...
from backend.models.dashboard_model import DashboardStats
...
def update_dashboard_stats(stat: DashboardStatCreate, db: Session = Depends(get_db)):
    existing = db.query(DashboardStats).filter(DashboardStats.date == stat.date).first()
    ...
    if existing:
        ...
        new_stat = DashboardStats(**stat.dict())
        db.add(new_stat)
